...
export class LoginauthComponent implements OnInit {
  ...
  auth(type) {
          ...
          error => {
            ...
            if (this.data.error.code === 500 && this.data.error.msg === 'user not verified') {
              ...
              return this.toastr.warning(
                'Please click on the Verification link sent to your Registered Email'
              );
            } else {
    ...
    if (type === 'signup') {
      ...
      const phone = this.registerForm.value.mobile;
      if (phone.toString().length < 7 && phone.toString().length > 12) {
        this.toastr.warning('Mobile number must be valid', null, {
          timeOut: 4000
        });
        return;
      }
  ...
  resendVerifyEmail() {
    this.authenticationService
      .resendVerifyEmail(this.f.email.value)
      .pipe()
      .subscribe(
        data => {
          this.data = data;
